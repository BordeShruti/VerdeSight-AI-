...
const AuthPage = () => {
  ...
  if (verificationEmail) {
    return (
      <div className="auth-surface min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md border-white/60 bg-white/95 shadow-2xl shadow-emerald-950/10">
          <CardHeader>
      ...
      <Card className="w-full max-w-md border-white/60 bg-white/95 shadow-2xl shadow-emerald-950/10">
        <CardHeader>
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-emerald-800">
            <span className="grid size-8 place-items-center rounded-lg bg-primary text-primary-foreground">V</span>
            VerdeSight AI
          </div>
          <CardTitle className="font-display text-2xl">{mode === "signup" ? "Create your ESG workspace" : "Welcome back"}</CardTitle>
          <CardDescription>
            {mode === "signup" ? "Set up a secure company workspace for ESG evidence and analysis." : "Sign in to continue to your ESG decision dashboard."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs
